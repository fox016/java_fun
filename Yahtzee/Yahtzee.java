...
public class Yahtzee extends JFrame implements ActionListener, ItemListener {
	...
	public Yahtzee() {
		...
		String highString = "0";
		try {
			get = new BufferedReader(new InputStreamReader(new FileInputStream("highScore.txt")));
			highString = get.readLine();
		}catch(IOException e) {}
		highScore = Integer.parseInt(highString);
	...
	public void actionPerformed(ActionEvent event) {
		...
		// If score is clicked
			...
			if(total > highScore) {
				...
				try {
					save = new PrintWriter(new FileOutputStream("highScore.txt"), true);
				}catch(IOException e) {}
				save.println(total);
				save.close();
				highScore = total;
    ...
    public static void main(String[] args) {
    	...
    	yahtzee.setTitle("Nate's Yahtzee");
    	yahtzee.setSize(WIDTH,HEIGHT);
    	...
    	yahtzee.setLocation(dim.width/2 - WIDTH/2, dim.height/2 - HEIGHT/2);
    }
}
